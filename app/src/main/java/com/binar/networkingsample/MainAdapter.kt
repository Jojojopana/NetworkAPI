...
import com.binar.networkingsample.HomeFragmentDirections
import com.binar.networkingsample.data.model.GetAllCarResponseItem
import com.binar.networkingsample.databinding.ItemBinding
            ...
            RecyclerView.ViewHolder(binding.root) {
                fun bind(data: GetAllCarResponseItem) {
                    binding.apply {
                        ...
                        binding.harga.text = data.price.toString()
                        root.setOnClickListener{
                            onItemClick.onClickItem(data)
                        }
                        tomboldetail.setOnClickListener{
                            val user = GetAllCarResponseItem(name = data.name, price = data.price)
                            it.findNavController().navigate(HomeFragmentDirections.actionHomeFragmentToDetail(user))
                        }
                    }
                }
            }
